...
| perspectiveIntensity    | `double?`              | `null`                         | 透视强度（近大远小）。 <br/> 为 `null` 时，将默认按尺寸自适应。                                                                                                                                                                                |
| zOffset                 | `double`               | `0.0`                          | 旋转支点在 Z 轴方向的偏移量。 <br/> 不为 0 时，widget 将绕 Z 轴上的偏移支点产生倾斜摆动的效果。 <br/> 例如： <br/> `-100`：支点位于 widget 后方 100 像素。 <br/> `100`：支点位于 widget 前方 100 像素。 <br/> `0`：支点位于 widget 中心。      |
...
[Tilt container widget list]: /zh/v4/docs/widgets/tilt-container
[手势优先级]: #手势优先级
